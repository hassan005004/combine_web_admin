                ...
                <ul class="mt-3">
                    @php
                        $links = [
                            ...
                            ['segment' => 'domains', 'name' => 'Doamins', 'route' => route('domains.index')],
                            ['segment' => 'pages', 'name' => 'Pages', 'route' => route('pages.index')],
                            ['segment' => 'faqs', 'name' => 'FAQs', 'route' => route('faqs.index')],
                            ['segment' => 'contents', 'name' => 'Content', 'route' => route('contents.index')],
                            ['segment' => 'notifications', 'name' => 'Notifications', 'route' => route(name: 'notifications.index')],
                            ['segment' => 'notification-settings', 'name' => 'Notification Settings', 'route' => route(name: 'notification-settings.index')],
                            ['segment' => 'user-devices', 'name' => 'User Devices', 'route' => route('user-devices.index')],
                            ['segment' => 'settings', 'name' => 'Settings', 'route' => route('settings.index')],
                        ];
                    @endphp
                    @foreach($links as $link)
                    <li class="pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 bg-linear-to-r @if(in_array(Request::segment(1), [$link['segment']])){{ 'from-violet-500/[0.12] dark:from-violet-500/[0.24] to-violet-500/[0.04]' }}@endif">
                        <a class="block text-gray-800 dark:text-gray-100 truncate transition @if(!in_array(Request::segment(1), [$link['segment']])){{ 'hover:text-gray-900 dark:hover:text-white' }}@endif" href="{{ $link['route'] }}">
                            <div class="flex items-center">
                                <svg class="shrink-0 fill-current @if(in_array(Request::segment(1), [$link['segment']])){{ 'text-violet-500' }}@else{{ 'text-gray-400 dark:text-gray-500' }}@endif" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                                ...
                                <span class="text-sm font-medium ml-4 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">{{ $link['name'] }}</span>
                            </div>
                        </a>
                    </li>
                    @endforeach
